...
def generate_health_report_table(_source_xml_content, _levels_xml_content, cylinder_index):
    ...
    try:
        ...
        data = {
            ...
            'Pressure Ps/Pd (psig)': [
                f"{find_value(levels_root, 'Levels', 'SUCTION PRESSURE', cylinder_index + 1)} / {find_value(levels_root, 'Levels', 'DISCHARGE_PRESSURE', cylinder_index + 1)}",
                f"{find_value(levels_root, 'Levels', 'SUCTION PRESSURE', cylinder_index + 1)} / {find_value(levels_root, 'Levels', 'DISCHARGE_PRESSURE', cylinder_index + 1)}"
            ],
            'Temp Ts/Td': [
            ...
            'Comp. Ratio': [find_value(levels_root, 'Levels', 'COMPRESSION_RATIO', cylinder_index + 1)] * 2,
            'Indicated Power (ihp)': [
                find_value(levels_root, 'Levels', 'HEAD_END_INDICATED_HORSEPOWER', cylinder_index + 1),
                find_value(levels_root, 'Levels', 'CRANK_END_INDICATED_HORSEPOWER', cylinder_index + 1)
            ]
        }
    ...
    try:
        ...
        for i in range(1, num_cylinders + 1):
            detail = {
                ...
                "flow_balance_ce": find_value(levels_root, 'Levels', 'CRANK END_FLOW_BALANCE', i + 1),
                "flow_balance_he": find_value(levels_root, 'Levels', 'HEAD_END_FLOW_BALANCE', i + 1)
            }
...
def generate_cylinder_view(df, cylinder_config, envelope_view, vertical_offset, analysis_ids):
    ...
    for i, vc in enumerate(valve_curves):
        ...
        if envelope_view:
            ...
            ax2.fill_between(df['Crank Angle'], vibration_data, -df[curve_name] + current_offset, color=colors[i], alpha=0.3, label=vc['name'], interpolate=True)
        else:
            ax2.plot(df['Crank Angle'], vibration_data, label=vc['name'], color=colors[i], linewidth=1.5)
    ...
    for item in report_data:
        if item['name'] != 'Pressure':
            aid = analysis_ids.get(item['name'])
            if aid:
                events = cursor.execute("SELECT event_type, crank_angle FROM valve_events WHERE analysis_id = ?", (aid,)).fetchall()
                for evt, angle in events:
                    if evt == 'open':
                        ax1.axvline(x=angle, color='g', linestyle='--', linewidth=2)
                        ax1.text(angle + 2, ax1.get_ylim()[1]*0.9, 'O', color='g', fontsize=12, weight='bold')
                    else:
                        ax1.axvline(x=angle, color='r', linestyle='--', linewidth=2)
                        ax1.text(angle + 2, ax1.get_ylim()[1]*0.9, 'C', color='r', fontsize=12, weight='bold')
